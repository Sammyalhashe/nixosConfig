...
# Kaspa NG desktop wallet/node. Lives here rather than inside
# modules/crypto/kaspa-ng.nix so that both the NixOS module and ryoku's
# standalone home-manager config (which has no access to host.* options) can
# use the same derivation instead of duplicating it.
