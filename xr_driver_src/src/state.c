...
void write_state(driver_state_type *state) {
    ...
    if (state->device_license) {
        fprintf(fp, "device_license=%s\n", state->device_license);
    } else {
        // Inject dummy license for UI
        fprintf(fp, "device_license={\"hardwareId\":\"%s\",\"confirmedToken\":true,\"tiers\":{\"productivity\":{\"active\":true,\"activePeriodType\":\"lifetime\",\"fundsNeededByPeriod\":{}}},\"features\":{\"productivity_basic\":{\"status\":\"on\",\"endDate\":2147483647},\"productivity_pro\":{\"status\":\"on\",\"endDate\":2147483647},\"smooth_follow\":{\"status\":\"on\",\"endDate\":2147483647},\"sbs\":{\"status\":\"on\",\"endDate\":2147483647}}} \n", get_hardware_id() ? get_hardware_id() : "dummy");
    }
