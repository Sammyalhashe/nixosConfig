...
let
  ...
  sops.secrets.filestore_nextcloud_admin_password = {};
  sops.secrets.filestore_authentik_secret = {};
  sops.secrets.filestore_vaultwarden_client_id = {};
  sops.secrets.filestore_vaultwarden_client_secret = {};
  sops.secrets.filestore_vaultwarden_admin_token = {};

  sops.templates."nextcloud.env".content = ''
    POSTGRES_PASSWORD=${config.sops.secrets.filestore_postgres_password.placeholder}
    NEXTCLOUD_ADMIN_PASSWORD=${config.sops.secrets.filestore_nextcloud_admin_password.placeholder}
  '';

  sops.templates."authentik.env".content = ''
    AUTHENTIK_POSTGRESQL__PASSWORD=${config.sops.secrets.filestore_postgres_password.placeholder}
    AUTHENTIK_SECRET_KEY=${config.sops.secrets.filestore_authentik_secret.placeholder}
  '';

  sops.templates."vaultwarden.env".content = ''
    SSO_CLIENT_ID=${config.sops.secrets.filestore_vaultwarden_client_id.placeholder}
    SSO_CLIENT_SECRET=${config.sops.secrets.filestore_vaultwarden_client_secret.placeholder}
    ADMIN_TOKEN=${config.sops.secrets.filestore_vaultwarden_admin_token.placeholder}
  '';
...
in
{
  ...
  virtualisation.oci-containers.containers = {
    ...
    nextcloud-app = {
      ...
      environmentFiles = [ config.sops.templates."nextcloud.env".path ];
      environment = {
  ...
  users = {
    ...
    users."${user}" = {
      isNormalUser = true;
      passwordFile = config.sops.secrets.filestore_user_password.path;
      extraGroups = [
        "wheel"
        "podman"
      ];
